...
@app.route("/recomend")
def recomend():
    return render_template('recomendsong.html')

@app.route('/rec', methods =['POST'])
def rec():
    username = request.form['username']
    recsong = request.form['recsong']
    genre = request.form['genre']
    cursor = conn.cursor()
    cursor.execute('INSERT INTO recsong VALUES ( % s, % s, % s)', (username,recsong,genre,))
    cursor.connection.commit()
    return render_template('index.html')
...
def recdis():
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM recsong")
    song_data = cursor.fetchall()
    cursor.close()
    return render_template('songdisplay.html',  song_data=song_data)
